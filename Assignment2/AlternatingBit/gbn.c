...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    /*new ack: the last packet sequence number*/
    /*window size: althernative bit window size can only 1*/
    if ((packet.acknum == (A_nextseqnum+1)%2) 
    && (windowcount >= WINDOWSIZE)){/**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      /* packet is a new ACK */
      if (TRACE > 0)
...
void B_input(struct pkt packet)
{
  ...
  if  ( (!IsCorrupted(packet))  && (packet.seqnum == expectedseqnum) ) {
    ...
    /*send the last packet ack number*/
    /*in alternative bit, it can only be 0 or 1*/
